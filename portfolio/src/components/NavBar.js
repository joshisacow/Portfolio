...
const NavBar = () => {
    return (
        <div className="fixed z-10 lg:absolute top-2 sm:top-4 w-full mx-auto h-16">
                ...
                <div className="bg-background/60 backdrop-blur-md backdrop-brightness-95 dark:backdrop-brightness-125 rounded-full px-2 py-2 sm:px-6 lg:px-8 flex items-center justify-center gap-2 text-sm sm:text-base lg:text-lg">
                    ...
                    {navigation.map((item) => (
                        <Link
                            key={item.name}
                            href={item.href}
                            className="relative rounded-3xl px-2 py-1 sm:px-3 sm:py-2 hover:bg-white/40 dark:hover:bg-white/10 hover:backdrop-blur-2xl hover:text-secondary"
                        >
                            {item.name}
                        </Link>
                    ))}
