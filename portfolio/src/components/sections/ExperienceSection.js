        ...
        <div className='mt-10'>
            ...
            {data && data.map((experience, index) => (
                <Link key={experience.id} href={experience.link ?? "/"} target="_blank" rel="noopener noreferrer">
                    <div className="shadow rounded-lg p-4 mb-2 group hover:bg-background2 hover:z-10 hover:-translate-y-1 hover:shadow-2xl transform transition-transform duration-300">
                        <h2 className='font-semibold group-hover:text-secondary'>{experience.company} | {experience.position}</h2>
